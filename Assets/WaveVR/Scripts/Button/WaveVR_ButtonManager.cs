...
public class WaveVR_ButtonManager : MonoBehaviour {
	...
	public HmdButtonOption m_HmdOptions = new HmdButtonOption();
	public ControllerButtonOption m_DominantOptions = new ControllerButtonOption();
	public ControllerButtonOption m_NonDominantOptions = new ControllerButtonOption();
	private ulong m_HmdOptionValue = 0, m_DominantOptionValue = 0, m_NonDominantOptionValue = 0;
	private void UpdateOptionValues()
	{
		this.m_HmdOptions.UpdateOptionValue();
		this.m_DominantOptions.UpdateOptionValue();
		this.m_NonDominantOptions.UpdateOptionValue();
	}
	...
	void Awake()
	{
		...
		UpdateOptionValues();
		m_HmdOptionValue = this.m_HmdOptions.optionValue;
		m_DominantOptionValue = this.m_DominantOptions.optionValue;
		m_NonDominantOptionValue = this.m_NonDominantOptions.optionValue;
	}
	...
	private void createDominantRequestAttributes()
	{
		...
		List<WaveVR_ButtonList.EButtons> list = new List<WaveVR_ButtonList.EButtons>();
		if (this.m_DominantOptions.Bumper)
			list.Add(WaveVR_ButtonList.EButtons.Bumper);
		if (this.m_DominantOptions.DPadDown)
			list.Add(WaveVR_ButtonList.EButtons.DPadDown);
		if (this.m_DominantOptions.DPadLeft)
			list.Add(WaveVR_ButtonList.EButtons.DPadLeft);
		if (this.m_DominantOptions.DPadRight)
			list.Add(WaveVR_ButtonList.EButtons.DPadRight);
		if (this.m_DominantOptions.DPadUp)
			list.Add(WaveVR_ButtonList.EButtons.DPadUp);
		if (this.m_DominantOptions.Grip)
			list.Add(WaveVR_ButtonList.EButtons.Grip);
		if (this.m_DominantOptions.Menu)
			list.Add(WaveVR_ButtonList.EButtons.Menu);
		if (this.m_DominantOptions.Thumbstick)
			list.Add(WaveVR_ButtonList.EButtons.Thumbstick);
		if (this.m_DominantOptions.Touchpad)
			list.Add(WaveVR_ButtonList.EButtons.Touchpad);
		if (this.m_DominantOptions.Trigger)
			list.Add(WaveVR_ButtonList.EButtons.Trigger);
		if (this.m_DominantOptions.VolumeDown)
			list.Add(WaveVR_ButtonList.EButtons.VolumeDown);
		if (this.m_DominantOptions.VolumeUp)
			list.Add(WaveVR_ButtonList.EButtons.VolumeUp);
	...
	private void createNonDominantRequestAttributes()
	{
		...
		List<WaveVR_ButtonList.EButtons> list = new List<WaveVR_ButtonList.EButtons>();
		if (this.m_NonDominantOptions.Bumper)
			list.Add(WaveVR_ButtonList.EButtons.Bumper);
		if (this.m_NonDominantOptions.DPadDown)
			list.Add(WaveVR_ButtonList.EButtons.DPadDown);
		if (this.m_NonDominantOptions.DPadLeft)
			list.Add(WaveVR_ButtonList.EButtons.DPadLeft);
		if (this.m_NonDominantOptions.DPadRight)
			list.Add(WaveVR_ButtonList.EButtons.DPadRight);
		if (this.m_NonDominantOptions.DPadUp)
			list.Add(WaveVR_ButtonList.EButtons.DPadUp);
		if (this.m_NonDominantOptions.Grip)
			list.Add(WaveVR_ButtonList.EButtons.Grip);
		if (this.m_NonDominantOptions.Menu)
			list.Add(WaveVR_ButtonList.EButtons.Menu);
		if (this.m_NonDominantOptions.Thumbstick)
			list.Add(WaveVR_ButtonList.EButtons.Thumbstick);
		if (this.m_NonDominantOptions.Touchpad)
			list.Add(WaveVR_ButtonList.EButtons.Touchpad);
		if (this.m_NonDominantOptions.Trigger)
			list.Add(WaveVR_ButtonList.EButtons.Trigger);
		if (this.m_NonDominantOptions.VolumeDown)
			list.Add(WaveVR_ButtonList.EButtons.VolumeDown);
		if (this.m_NonDominantOptions.VolumeUp)
			list.Add(WaveVR_ButtonList.EButtons.VolumeUp);
